...
class IrisPlantVisualizer:
    def __init__(
        ...
        self.rational_forward_kin = rational_forward_kin
        ...
        self.meshcat_nontc = StartMeshcat()
        self.meshcat_nontc.Delete()
        builder_nontc = DiagramBuilder()
        plant_nontc, scene_graph_nontc = AddMultibodyPlantSceneGraph(
            builder_nontc, time_step=0.0)
        plant_nontc.Finalize()
        self.visualizer_nontc = MeshcatVisualizer.AddToBuilder(
            builder_nontc, scene_graph_nontc, self.meshcat_nontc)
        ...
        self.nontc_diagram = builder_nontc.Build()
        self.nontc_diagram_context = self.nontc_diagram.CreateDefaultContext()
    ...
    def show_res_q(self, q):
        ...
        self.plot_nontc_points(np.array(q), name='/q', color=color, radius=0.05)
    ...
    def visualize_collision_constraint(self, **kwargs):
        ...
        self.visualize_collision_constraint_nontc(**kwargs)
    
    def visualize_collision_constraint_nontc(self, **kwargs):
        if self.plant.num_positions() == 3:
            self._visualize_nontc_collision_constraint3d(**kwargs)
        else:
            self._visualize_nontc_collision_constraint2d(**kwargs)

    def _visualize_nontc_collision_constraint3d(
            self,
            N=50,
            factor=2,
            iso_surface=0.5,
        ...
        self.meshcat_nontc.SetObject("/collision_constraint_q",
    ...
    def _visualize_nontc_collision_constraint2d(self, factor=2, num_points=20):
        s0 = np.linspace(
            factor *
    ...
    def plot_nontc_points(self, points, name, **kwargs):
        if len(points.shape) == 1:
            viz_utils.plot_point(points, self.meshcat_nontc, name, **kwargs)
        else:
            for i, s in enumerate(points):
                viz_utils.plot_point(
                    s, self.meshcat_nontc, name + f"/{i}", **kwargs)
