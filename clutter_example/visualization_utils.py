...
def draw_nontc_traj(meshcat_instance, traj, maxit, name = "/trajectory",
              color = Rgba(0,0,0,1), line_width = 3, q_star=None):
    if q_star is None:
        q_star = np.array([0.0, 0.0])
    pts_unsqueezed = 2*np.arctan(np.array([traj.value(it * traj.end_time() / maxit) for it in range(maxit)]))
    pts = np.squeeze(pts_unsqueezed) + q_star
    pts_3d = np.hstack([pts, 0 * np.ones((pts.shape[0], 3 - pts.shape[1]))]).T
    meshcat_instance.SetLine(name, pts_3d, line_width, color)
