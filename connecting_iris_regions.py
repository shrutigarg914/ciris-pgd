...
all_regions = [r for r in regs.values()]
def visualise_sample(region, prev=None, seed=42):
    if prev is None:
        print("using chebyshev center")
        prev = region.ChebyshevCenter()
    
    rng = RandomGenerator(seed)
    sample = region.UniformSample(rng, prev)
    q = Ratfk.ComputeQValue(sample, q_star)
    plant_context = plant.GetMyContextFromRoot(context)
    plant.SetPositions(plant_context, q)
    diagram.ForcedPublish(context)
    return sample

def visualise_s_point(spoint):
    q = Ratfk.ComputeQValue(spoint, q_star)
    plant_context = plant.GetMyContextFromRoot(context)
    plant.SetPositions(plant_context, q)
    diagram.ForcedPublish(context)
